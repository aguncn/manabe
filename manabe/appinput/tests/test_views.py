...
class AppInputListTests(TestCase):
    def setUp(self):
...
class AppInputDetailTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username='test',
            email='test@example.com',
...
class AppInputCreateTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username='test',
            email='test@example.com',
    ...
    def test_detail_url_resolves_home_view(self):
        view = resolve('/app/create/')
        self.assertEqual(view.func.__name__, AppInputCreateView.as_view().__name__)


class AppInputUpdateTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username='test',
            email='test@example.com',
    ...
    def test_detail_view_status_code(self):
        url = reverse('appinput:edit', kwargs={'pk': self.new_app.id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_detail_url_resolves_home_view(self):
        view = resolve('/app/edit/1/')
        self.assertEqual(view.func.__name__, AppInputUpdateView.as_view().__name__)